...
class Gerber
{
    ...
    public $extract_path = "gerber/extract";
    ...
    //relative to the script directory
    public $image_path = 'gerber/image';

    //size of images to output, in pixels
    public $image_size = "300x300";    //'500x500'
    ...
    public function __construct($gerberPath)
    {
        $this->gerber_path = $gerberPath;
    }
    ...
    {
        return [
            'detected_number' => $board['number_format'],
            'detected_coordinate_mode' => $board['coordinate_mode'],
            'count' => 2,
            'units' => $board['units'],
            'x_dir' => [
                'min' => $board['x_min'],
    ...
    */
    ...
    function boardCompositImage($name, $side, $color_silk)
    {
        ...
        if (!$base) { //copy could not create image image, return
            $base = $this->getImage('no_imgae.png');
